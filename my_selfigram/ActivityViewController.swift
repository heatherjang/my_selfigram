...
//  my_selfigram
//
//  Created by Heather on 2016-09-05.
...
class ActivityViewController: UITableViewController {
    ...
    override func viewDidAppear(animated: Bool) {
        guard let query = Activity.query() else{
            return
        }
        query.orderByDescending("createdAt")
        query.includeKey("user")
        query.includeKey("post.user")
        query.findObjectsInBackgroundWithBlock{ (response, error) in
            if let activities = response as? [Activity]{
                // Store new records
    ...
    override func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
        ...
        if let liker = activity.user.username,
            let liked = activity.post.user.username {
            cell.textLabel?.text = "♥️ \(liker) liked \(liked)'s photo"
        }
